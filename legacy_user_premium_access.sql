-- Legacy User Premium Access Implementation
-- Grants premium access to users created before July 20, 2025 until September 1, 2025

-- Update the subscription status function to include legacy user support
...
-- Verification query - Check how many legacy users will be affected
-- (Uncomment to run this query for verification)
/*
SELECT 
    COUNT(*) as total_legacy_users,
    COUNT(CASE WHEN created_at < '2025-07-20 00:00:00+00' THEN 1 END) as users_before_cutoff
FROM auth.users 
WHERE created_at < '2025-09-01 00:00:00+00';
*/

-- Test query - Check legacy user status for a specific user
-- (Replace 'USER_UUID_HERE' with actual user UUID to test)
/*
SELECT * FROM get_user_subscription_status('USER_UUID_HERE'::UUID);
*/ 